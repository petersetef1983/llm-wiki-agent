...
1. Classify the material into an existing theme, `shared/`, or inbox review. `inbox/` is staging only, not the final knowledge structure.
2. Inspect structure with `inventory` or `classify-inbox` only when destination or content kind is unclear.
